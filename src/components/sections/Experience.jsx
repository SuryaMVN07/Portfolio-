...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-20 sm:py-32 px-6 max-w-4xl mx-auto relative z-10">
      <h2 className="text-center text-5xl font-black mb-20 reveal uppercase tracking-[0.4em] italic text-purple-400">{t.heading}</h2>
      <div className="space-y-12">
        {EXPERIENCE.map((exp, i) => {
          const translated = t.items[i];
          return (
            <div key={i} className="relative pl-12 border-l-2 border-purple-500/30 reveal">
              ...
              <div className="bg-white/5 p-8 sm:p-12 rounded-[2rem] border border-white/10 hover:bg-white/[0.08] transition-all group shadow-xl">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                  <div>
                    <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tighter group-hover:text-purple-400 transition-colors text-white">
                      {translated?.role ?? exp.role}
                    </h3>
                    {/* Company is a proper noun — kept in English */}
                ...
                <p className="text-gray-400 text-sm font-medium italic leading-relaxed">
                  {translated?.description ?? exp.description}
                </p>
                {exp.url && (
                  <a href={exp.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-6 text-[10px] text-purple-400 font-black uppercase tracking-widest border-b border-purple-400/30 hover:border-purple-400 transition-all">
                    {t.live} <ExternalLink size={12}/>
                  </a>
                )}
              </div>
            </div>
